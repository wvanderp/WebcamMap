...
function FilterButton({
	filters, setFilter, index, text = undefined
}: FilterButtonProps) {
	const handleToggle = () => setFilter((prev: Filters) => {
...
function MaintenancePage() {
	...
	const filterRow = [
		FilterButton({
			filters, setFilter: setFilters, index: 'youtube', text: 'YouTube'
		}),
		FilterButton({
			filters, setFilter: setFilters, index: 'noName', text: 'No Name'
		}),
		FilterButton({
			filters, setFilter: setFilters, index: 'noDescription', text: 'No Description'
		}),
		FilterButton({
			filters, setFilter: setFilters, index: 'me', text: 'My Edits'
		}),
		FilterButton({
			filters, setFilter: setFilters, index: 'fixme', text: 'Fixme'
		}),
		FilterButton({
			filters, setFilter: setFilters, index: 'unavailable', text: '404'
		}),
		FilterButton({
			filters, setFilter: setFilters, index: 'badYoutube', text: 'Bad YouTube link'
		}),
		FilterButton({
			filters, setFilter: setFilters, index: 'invalidUrl', text: 'Invalid URL'
		}),
		FilterButton({
			filters, setFilter: setFilters, index: 'duplicate', text: 'Duplicate'
		})
	];
