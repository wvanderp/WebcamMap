...
export default async function lintYoutube() {
  ...
  const youtubeLinksFull: Webcam[] = webcams.filter((a: Webcam) =>
    a.url.includes("youtube"),
  );
  const youtubeLinksShort: Webcam[] = webcams.filter((a: Webcam) =>
  ...
  const youtubeLinksToCheck = youtubeLinks.filter((a) => {
    ...
    return (
      Date.now() - lastCheck > 30 * 24 * 60 * 60 * 1000 || ytIndex[a.url].bad
    );
  });
